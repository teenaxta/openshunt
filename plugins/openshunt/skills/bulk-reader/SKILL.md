---
name: bulk-reader
description: Delegate large file reading and surface-level multi-file questions to a configured smaller native subagent. Use OpenShunt for bounded tasks.
---

Resolve `../../scripts/openshunt.mjs` relative to this skill file. From the project directory run:

```sh
node "<plugin-root>/scripts/openshunt.mjs" bulk-read --host <claude-or-codex> --question "Which methods access the database?" --paths src/service.ts
```
